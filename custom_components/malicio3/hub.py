...
class Hub(DataUpdateCoordinator):
    ...
    def __init__(
        ...
        self._zoneId = -1
        self._access_token = ""
        self._id = "malicio3." + username.lower()
        ...
        if "name" in modelInfos:
            self.device_info = DeviceInfo(
                ...
                identifiers={(DOMAIN, "malicio3" + str(deviceId))},
        ...
        if self._test_load:
            ...
            self.online = True
            with open(
                self._hass.config.config_dir + "/malicio3_eoras2.json",
                encoding="utf-8",
            ) as json_file:
    ...
    def update_devices_from_json_data(self, json_data) -> None:
        ...
        if self._dump_json:
            with open(
                self._hass.config.config_dir + "/Malicio3.json", "w", encoding="utf-8"
            ) as outfile:
